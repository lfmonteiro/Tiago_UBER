...
<head>
  ...
  <style>
    ...
    .container { max-width: 520px; margin: 0 auto; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.08); }
    ...
    .label { color: #888; }
    .value { color: #111; font-weight: 500; text-align: right; max-width: 60%; }
    .btn { display: inline-block; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 14px; margin: 8px 8px 0 0; }
...
<body>
  <div class="container">
    ${conteudo}
    <div class="footer">Este e-mail foi gerado automaticamente pelo sistema de agendamentos.</div>
  </div>
</body>
</html>`;
}
...
async function emailMotoristaNovoPedido(motorista, agendamento) {
  ...
  const html = htmlBase(`
    <div class="header">
      ...
      <p>Um passageiro solicitou uma corrida particular</p>
    </div>
    <div class="body">
      <div class="row"><span class="label">Cliente</span><span class="value">${agendamento.cliente.nome}</span></div>
      <div class="row"><span class="label">WhatsApp</span><span class="value">${agendamento.cliente.telefone}</span></div>
...
async function emailClienteConfirmacao(agendamento) {
  ...
  const html = htmlBase(`
    <div class="header">
      <h1>✅ Corrida confirmada!</h1>
      <p>Seu motorista particular confirmou o agendamento</p>
    </div>
    <div class="body">
      <div class="row"><span class="label">Data</span><span class="value">${data} às ${agendamento.horario}</span></div>
      <div class="row"><span class="label">Origem</span><span class="value">${agendamento.origem}</span></div>
  ...
  try {
    await transporter.sendMail({
      ...
      subject: `✅ Corrida confirmada — ${data} às ${agendamento.horario}`,
      html
    });
    return { sucesso: true };
  } catch (error) {
